...
void MainComponent::drawPanel(juce::Graphics& g, juce::Rectangle<int> bounds, const juce::String& title)
{
    ...
    g.drawText(title, bounds.removeFromTop(24).reduced(8, 0),
...
void MainComponent::paint(juce::Graphics& g)
{
    ...
    g.drawText("Sine Wave Synth", juce::Rectangle<int>(0, 0, getWidth(), 36),
...
void MainComponent::resized()
{
    ...
    // Oscilloscope panel sits just below the title
    scopePanel = juce::Rectangle<int>(padding, titleHeight + padding,
                                      getWidth() - padding * 2, scopeHeight);
    oscilloscope.setBounds(scopePanel.reduced(4));

    // ADSR and filter panels sit below the scope
    ...
    filterTypeButton.setBounds(filterInnerX,
                               filterInnerY + labelHeight + filterSliderH + padding,
                               filterInnerW, buttonHeight);
...
{
    if      (slider == &attackSlider)    adsrParams.attack  = (float)attackSlider.getValue();
    else if (slider == &decaySlider)     adsrParams.decay   = (float)decaySlider.getValue();
    else if (slider == &sustainSlider)   adsrParams.sustain = (float)sustainSlider.getValue();
    else if (slider == &releaseSlider)   adsrParams.release = (float)releaseSlider.getValue();
    else if (slider == &cutoffSlider)    cutoffHz           = (float)cutoffSlider.getValue();
    else if (slider == &resonanceSlider) resonance          = (float)resonanceSlider.getValue();
...
void MainComponent::getNextAudioBlock(const juce::AudioSourceChannelInfo& bufferToFill)
{
    ...
    // Feed the rendered audio into the oscilloscope
